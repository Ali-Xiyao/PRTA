...
| Error | Attempt | Resolution |
|---|---:|---|
| The first terminal-time summarizer passed compilation and produced the read-only terminal-duration inventory, but Ruff found import-order drift before the script was promoted as a reusable monitor | 1 | No runtime artifact was written and no intermediate metric was read. Apply only Ruff's import fix, rerun format/lint/compile, and retain the estimator under private untracked runtime; derive ETA only from terminal receipt timestamps. |
| The first read-only terminal-metadata auditor passed compilation and executed correctly but Ruff reported import ordering plus formatter drift | 1 | The receipt audit itself was read-only and returned the terminal hash/identity without intermediate metrics. Apply only Ruff's mechanical import/format fixes, expand the safe identity allowlist, then repeat compile/lint/format, local/remote hash, and the same terminal audit. |
| The first no-tail-S43 terminal-receipt field audit embedded a base64 Python `-c` payload through nested PowerShell/SSH quotes; PowerShell split the payload and the remote shell rejected it | 1 | The separate `sha256sum` did not execute and no receipt changed. Use a small read-only auditor file created with `apply_patch`, transfer it to `/tmp`, verify its SHA/compile, and invoke it with the exact terminal-receipt path rather than retrying inline quoting. |
| The first 11:37 heartbeat local status probe repeated the known Windows PowerShell pattern of piping directly from a `foreach` block and failed at parse time | 2 | No status read or mutation occurred. Reissued with explicit result arrays before the pipeline; all three exact local lane-progress surfaces, processes, GPUs, disk, failure guards, and zero protected reads verified. Never pipe directly from `foreach` in this shell again. |
| The first 11:37 heartbeat server status probe used remote shell loop variables inside a PowerShell double-quoted SSH command, so PowerShell erased them and the remote shell rejected the command before any check ran | 1 | No remote state changed. Reissued every completion/failure check as an explicit absolute path without a remote variable or loop; both active Slurm children, both waiting watchers, stopped parents, telemetry, identities, disk, failure guards, and zero protected reads verified. |
...
| First Slim exporter gate found three overlength Chinese narrative literals and the combined test command guessed nonexistent `tests/test_slim_finalize.py` | 1 | The fallback ran all seven real Slim export/matrix tests successfully. Wrap the narrative literals and rerun only existing focused files before the full repository gate. |
