...
- Both idle RTX3090s are now occupied only by the new frozen native-baseline
  ...
  intermediate metric.
# 2026-08-14 allocation 3066 comparison-expansion decision gate

- The user explicitly authorized placing part of the still-missing, main-method-independent comparison work on allocation 3066. This is a new execution authorization, but it does not authorize duplicate cells, outcome-adaptive selection, protected-data access, or interruption of any active Wave045 checkpoint diagnostic.
- The currently frozen Wave046 remainder is already exact and minimal: new B401 seeds 17/28/43 plus B402 seeds 28/43, reusing terminal B402-S17 and B403-S17/S28/S43. At launch, B401-S17 and B401-S28 were active locally, while B401-S43 then B402-S28/S43 were planned in fixed local order.
- Before any redistribution, the live 3066 scientific-child status must be checked because the most recent verified state had `W045D-V5-S17` active at step `3066.104`. A Wave046 cell may use 3066 only after that diagnostic has a terminal PASS receipt and the allocation has no other scientific child.
- Because Wave046's preparation froze local RTX3090 assignments, any safe use of 3066 requires a separately hashed, pre-outcome execution amendment that claims only still-unstarted cells, prevents the original local supervisor from launching those cells, preserves their configs/seeds/budgets byte-for-byte, records A800 hardware explicitly, and keeps aggregation hardware-stratified. If an exact no-duplicate handoff cannot be made fail-closed, the correct state is HOLD rather than concurrent duplicate launch.
- The live Wave046 queue confirms exactly two RUNNING cells (`W046-B401-S17` on local GPU0 and `W046-B401-S28` on local GPU1) and exactly three still-PLANNED cells (`W046-B401-S43`, `W046-B402-S28`, `W046-B402-S43`). No cell is terminal yet, so the unstarted handoff set is still outcome-independent.
- A direct invocation of the local copy of the Wave045 diagnostic controller's `status-server` mode returned null progress for both server workers. This is not evidence that 3066 is free; it indicates that this local status surface does not currently resolve the canonical server runtime. The next check must use the already established canonical SSH/runtime path and process/Slurm surfaces before any launch.
- The canonical server diagnostic runtime is explicitly rooted at `/ipfs/inspurfileset/home/dqxy/dqxy11/projects/xiyaowang/050_VisualVIT/PRTA-CXR/data/runtime/server_runs/continuous_lightweight_dev_search_v1/wave045_postreview_mechanism_diagnostics_v1`, while the local controller process resolved that POSIX path against Windows. This explains the null local result and defines the exact allowlisted remote path for the live check.
- The canonical controller is deployed under the namespace's `experiments/` directory. Its formal status now shows server3066 fully terminal: V3/V4/V5 diagnostic receipts all PASS, with the newly terminal `W045D-V5-S17` receipt SHA `65c533386a44d42046a8498a042b341b7e2fa8079e2ab9ef83b15b1878a2e412`, no failure, and zero protected reads. Thus Wave045 no longer owns a scientific child on 3066; a final Slurm/GPU occupancy check remains before handing off comparison work.
- Slurm confirms allocation 3066 is RUNNING on `gpu01` with only the retained parent batch step and telemetry step `3066.2`; no scientific step remains. This clears the one-scientific-child occupancy gate without cancelling either retained process.
- The Wave046 local supervisor is the generic two-device `scripts/07b_run_development_queue.py` process (PID 12516), polling the mutable `run_queue.json` every 30 seconds. Therefore a planned-cell handoff must account for its refill behavior before either local device becomes free; simply launching the same ID on 3066 would create a duplicate.
- The visible wrapper only dispatches into `prta_cxr.cli_queue`; scheduler behavior must be audited in that module before choosing between a fail-closed queue amendment and a replacement continuation supervisor. No process has been suspended or modified yet.
- The scheduler re-reads `run_queue.json` atomically on every poll and selects only rows whose status is exactly `PLANNED`. This permits a fail-closed pre-outcome claim amendment without touching active children: change only the outsourced row to a non-launchable externally claimed status (or remove it under a separately hashed queue contract), while retaining the two RUNNING rows and two local B402 rows.
- `W046-B401-S43.json` is path-free and already freezes the exact scientific identity (current-only family, tail4/rank32/H0, class-balanced focal loss, seed43, fixed budget, all PRTA auxiliary weights zero). Server path rematerialization can therefore occur entirely through CLI data/cache/weights/output arguments without changing this config's byte SHA. The Wave046 scientific source remains commit `62235ff46fb26e4ccf05e3c9073188a84ca39119`.
- The queue runner re-reads the queue each poll, adopts existing RUNNING rows by PID/receipt, and launches only exact `PLANNED` rows. The least invasive safe split is therefore: atomically freeze an amendment receipt, mark only B401-S43 as externally claimed before either local device frees, keep active B401-S17/S28 unchanged, and allow local B402-S28/S43 to refill normally. The final queue controller must explicitly recognize the external claim so its completion receipt still accounts for all five cells.
- The existing Wave046 builder already verifies cleaned split/cache/weights/quality hashes, exact native-baseline budget equivalence, reused B402/B403 receipts/checkpoints, and a no-selection matrix. The 3066 amendment should reuse those immutable preparation facts rather than rerun scientific selection or reopen historical results.
- The server launch pattern is already proven: one blocking `srun --jobid=<allocation> --overlap --ntasks=1` child, exact source snapshot as working directory, formal authorization/environment identities, and `scripts/sues_hpc_run_dev_search_arm.sh`. A single-cell Wave046 server controller can reuse this pattern and independently verify its terminal training receipt.
- Wave046's immutable preparation binds source `62235ff`, queue SHA `01bfda09...7828`, split `45985f4f...8f89`, cleaned freeze `aa761c13...d069`, cache manifest `7bec0eb4...57d5`, training store `050a4837...e540`, text cache `1846e3d9...a3fd`, weights `52cc993c...76be`, quality audit `b6c7d4cc...b2c9`, no outcome selection, and zero protected reads. The server gate must match these exact scientific input hashes.
- Canonical-server preflight found the exact compatible cache at `formal_program_v1/cache/full_repartition_v1`; its manifest, text cache, training-store identity, and weights identity exactly match Wave046. The split, cleaned freeze, quality audit (at the server-search authority path), and live weights also match. Source snapshot `62235ff` is not yet present and must be transferred as a new immutable directory.
- Allocation 3066's visible GPU is an idle NVIDIA A800 80GB (10 MiB used, 0% utilization) with roughly 986 GB node memory free and about 1.25 TB filesystem space free. Node-wide process inspection still shows the separate allocation-9929 `W045-V5-S43` process, but 3066's allocated GPU remains empty; this is expected on the shared four-GPU node and does not conflict with the one-child-per-allocation rule.
- Repository rules require both `--formal` and the exact `PRTA_CXR_ALLOW_FORMAL` authorization for the server child, all real runtime artifacts to remain outside Git, and engineering checks before handoff. The amendment controller/package will be tracked only as source; configs, receipts, logs, checkpoints, and copied runtime data stay in the private Wave046 amendment namespace.
- During the second fail-closed amendment attempt, the live queue had independently changed: `W046-B401-S17` was now `FAILED`, while B401-S28 remained RUNNING and all three unstarted cells remained PLANNED. The amendment created no namespace and changed no queue. Redistribution is paused until the failure is classified from allowlisted failure/stderr/process surfaces; only an identity-preserving infrastructure retry in a new namespace is eligible.
- The B401-S17 failure surface says only `process exited without training receipt`; both stdout and stderr are zero bytes. The run had retained best/last checkpoints and its allowlisted progress file continued updating until about 19 seconds before the scheduler recorded failure, while GPU0 is now idle. No application/NVIDIA event-log error was found. This pattern is consistent with abrupt external/process-level termination rather than a reported scientific exception, but the cause is not yet proven; the partial namespace remains immutable and must never be resumed or overwritten.
- The generic queue runner's liveness check is correct and the run registry still marks B401-S17 `RUNNING` with no end/checkpoint/metrics path, proving the training process never reached its exception handler or terminal-receipt write. This is an identity-preserving infrastructure failure eligible for one fresh retry namespace under the standing rules.
- The amended resource plan is therefore failure-driven, not outcome-driven: allocation 3066 will run the exact B401-S17 config from scratch in a new immutable retry namespace; local GPU1 keeps B401-S28 untouched, while a replacement local continuation will claim still-unstarted B401-S43 and B402-S28/S43 and may immediately use now-free GPU0. The failed partial output is retained immutable and excluded from the final matrix except as infrastructure audit history.
- The first packaging attempt reached only a temporary outside-Git directory and failed because the frozen source engine is `src/prta_cxr/training/engine.py`, not `training.py`. The live queue was verified unchanged. That temporary directory has been moved intact to `wave046_native_baseline_3066_amendment_prepare_attempt1_failed` with failure-receipt SHA `c832e6377c8407f85f8b9ae2f18170aec0bc30a89def81242e260b4402b6af24`; the next preparation uses a new namespace.
- Amendment attempt2 successfully split the queue without duplication. The local continuation now runs B401-S28 and B401-S43 concurrently on GPU1/GPU0 from exact source `62235ff`, leaving both B402 cells planned. Thus allocation 3066 can safely own only the fresh B401-S17 retry; the original failed partial and its old PID are no longer eligible for any local refill.
- The server retry is now genuinely active, not merely scheduled: controller PID2045193 survived SSH disconnect and owns Slurm step `3066.106` for exact B401-S17. Only the preserved telemetry and parent steps coexist on allocation 3066. This completes the requested comparison-work placement without duplicating a cell or changing scientific inputs.
- The post-launch recheck preserves the same exact assignment: A800/3066 runs only B401-S17, local GPU1 runs B401-S28, local GPU0 runs B401-S43, and B402-S28/S43 remain the only planned successors. All exposed protected-data flags are false/zero, and no terminal or intermediate outcome was used for scheduling.
- The recurring monitor has been updated in place rather than duplicated. Its frozen prompt records the server/local controller identities, immutable infrastructure-failure history, Train/Dev-only boundary, one-child allocation rule, and hardware-stratified no-selection aggregation requirement.
- Wave045 has crossed its final execution barrier: all 18 original cells and all nine preregistered checkpoint diagnostics are terminal across the four fixed lanes. The last original receipt is `f6a65f89...c33c`; the final three diagnostic receipts are `290c1ac0...dad4`, `43157727...d07`, and `e47e84e...2c81`. Every formal worker reports no failure and zero protected reads.
- This completion authorizes only the preregistered independent verification and two separate no-selection aggregates. It does not authorize winner selection, H4 combination, protected evaluation, or reuse of intermediate metrics.
- Final three-seed Dev means are V0 `0.547960`, V1 `0.549378`, V2 `0.551821`, V3 `0.540140`, V4 `0.491089`, and V5 `0.525179` Macro-F1. These are descriptive no-selection results; V2 has the largest observed mean but is not designated a winner.
- Paired cumulative Macro-F1 deltas are V1−V0 `+0.001417`, V2−V1 `+0.002443`, V3−V2 `−0.011681`, V4−V3 `−0.049051`, and V5−V4 `+0.034090`. V4 has large cross-seed instability driven by its Seed28 terminal result and must not be repaired post hoc by seed deletion.
- Matched-hard, null, and reversed PRIOR interventions cause substantial prediction flips and negative Macro-F1 deltas for V3-V5. This demonstrates material PRIOR dependence, but it does not establish that the added dependence improves the main task.
...
- The new Slim request freezes the structural core (finding query, cross-time attention, temporal relation residual) plus ODC and matched-hard CMCP, fixes DMW off, and varies only prototype CE/state anchor in a 2x2. Because the attachment also forbids selecting on the repeatedly viewed Dev, the scientifically valid minimal matrix needs a common Train-only cell for S0 as well as S1-S3; the historical Dev-based w/o-DMW result is descriptive evidence, not a selection-comparable reuse.
- The prespecified simplification objective is “performance within tolerance, then simplest”: Macro-F1 within 0.003 of the best, ODER degradation no more than 0.0005, and no per-class recall loss over 0.01. These tolerances and tie-breaking must be frozen before outcomes are read.
- Existing reusable surfaces: `phase16_program.py` already freezes immutable configs/jobs with dependency-aware balanced queues; `phase16_queue_runner.py` provides resumable single-lane execution; the older `ifusion_queue.py` is coupled to four historical lane names and the original split, so the Slim run should use a dedicated small controller instead of mutating historical protocols.
- Training already supports independently zeroing `loss_weights.prototype_alignment` and `loss_weights.state`, while `model.components.selective_state_anchor` controls the corresponding state branch and `matched_hard_cmcp` plus `cmcp.matching=offline_hard_v1` enforces the fixed CMCP mechanism. A Slim builder can therefore be configuration-only scientifically, with new split/selection/receipt guards around existing training.
- The training dataset consumes rows by the literal `split` values `train` and `dev`, so a new Train-only selection surface can be represented as a sealed derived manifest whose original Train patients are deterministically reassigned to inner `train`/`dev`. The original cleaned-split freeze and cache remain potential blockers because the CLI may bind the original manifest hash; this must be audited before choosing the builder contract.
- The currently armed closeout waits directly for both corrected completion files, then finalizes and creates STOP. It must be replaced before Slim launch with a supervisor that waits for Phase16, runs the Slim lanes and Slim finalizer, exports safe results, then creates STOP; merely starting Slim beside it would race the global stop marker.
- The corrected-lane supervisors begin only after the main retry lane completion, making the boundary between main completion and corrected replay a clean insertion point for a prioritized Slim matrix without interrupting a currently running training process.
- `require_cleaned_manifest` accepts only the exact frozen original Train/Dev path/hash, so passing a derived Inner-Train/Slim-Dev manifest to the current CLI would correctly fail. The rigorous implementation needs a separate derived-selection receipt validator and an explicit CLI flag; it must prove every derived row came from original `split=train`, preserve row contents except `split`, preserve all source Train rows exactly once, and maintain patient disjointness.
- Matched-hard maps are bound to the split-manifest SHA and require candidates within each split. Therefore the Slim preparation must rebuild a new matched-hard map against the derived Train-only manifest and existing cached current embeddings; reusing the old map would be invalid even if sample IDs overlap.
- To minimize compute while retaining a new selection surface, use one frozen patient-level 80/20 Inner-Train/Slim-Dev split and all 4 arms × Seeds 17/28/43 = 12 comparable cells. Three-fold CV would triple this to 36 cells and is not the requested minimum.
- Training receipts retain per-epoch `macro_f1`, `opposite_direction_error_rate`, `min_class_recall`, and the full `ordinary` metric object. The Slim finalizer can select the recorded best epoch for each seed and aggregate all required frozen-rule metrics without any new Dev inference.
- User explicitly froze strict priority: only the two already-running plausible-noise cells may finish before Slim; every other main/corrected Phase16 cell must remain behind it. The safe mechanism is to `SIGSTOP` only each queue runner parent while its training child continues, run Slim in new overlapping Slurm steps after both GPUs become idle, then `SIGCONT` the parents only after Slim aggregation passes.
- Strict priority is now enforced on compute node `gpu01`: queue parents `2960860` (lane 9929) and `2960861` (lane 3066) were command-line/child validated and placed in Linux state `T` with SIGSTOP. Their active training children `229877` (S28) and `198437` (S17) remained `Rl`, so training continues while no later Phase16 cell can launch.
- Server inputs for Slim are fully available without protected surfaces: the three frozen Wave045 V2 configs live under `wave045_prta_v2_tail8_h0_v1/experiments/configs`, and Phase16's structural input manifest supplies the cleaned Train/Dev path, cleaned freeze/root, Block-4 Tail8 cache, text cache, BiomedCLIP weights, and label-quality audit. Current S17/S28 training receipts are still absent, so both active cells are genuinely running while their queue parents remain parked.
- Slim frozen source identity is commit `db52b32190b81d5cadcbbcd4c8011158b3c2f436`, deployed from archive SHA `4ea4ef20e2dc95249cfc426aad8b07de0ab296463d340bbf65828ee4dfbb78df` into a new server snapshot without modifying the live Phase16 source.
- Frozen server Slim preparation identity: preparation `3b1b0a1d8dd73bb9a4daaf8d78bffff8ef108102b3454f3ac2329b8712f336ca`, derived manifest `7fa8b46b...`, selection receipt `a9321374...`, queues `afc96308...`/`e92ec596...`. Estimated loads are 92,700/91,500 seconds (1,200-second imbalance) for one shared map plus 12 training cells.
- Slim attempt1 is a clean pre-training failure: the shared map job is the only FAILED job; every training cell is SKIPPED through dependency state, with no Slim checkpoint or metric produced. This preserves the frozen scientific matrix and permits a root-cause repair in a new immutable namespace without rerunning or deleting any result.
- The path repair is isolated to placeholder semantics: `{runtime_root}` now denotes the immutable Slim program root (configs/selection), while `{output_root}` remains the attempt-specific results root. Commit `456fb11...`, archive `b38fb732...`, and `slim_matrix_v2` preserve the exact arm/seed/split/selection contracts from v1.
- Slim v2 recovered correctly from the pre-training attempt1 failure: the first shared job is now RUNNING under new results/shared-state roots, proving `{runtime_root}` resolves the frozen program assets while `{output_root}` stays attempt-local. No v1 state or artifact was overwritten.
- The v2 shared map subsequently passed and both A800 lanes entered the frozen S0 seeds concurrently. At 13:15 CST each first cell recorded 1,600 of 80,400 optimizer steps, status RUNNING, with no failure/skip. Extrapolating only this observed initial throughput gives roughly 6.7 hours per full 20-epoch cell and 40 hours for six sequential cells per GPU, so 36–42 hours is the appropriate conservative matrix ETA rather than the earlier nominal 25-hour load estimate.
- The automatic result handoff is now an active hidden local process (PID 10592), not merely a prepared script. Its startup log exists, and its guarded isolated-clone design ensures an incomplete/failed Slim run cannot create or push paper result artifacts.
