...
def deterministic_patient_folds(
    ...
    *,
    labels: Sequence[str],
    fold_count: int = 5,
    salt: str = "prta-cxr-patient-folds-v1",
) -> dict[str, int]:
    if fold_count < 2:
    ...
    *,
    ...
    for fold in range(fold_count):
        selected = [
            row
            for row in records
            if assignment[str(row["patient_id_hash"])] == fold
        ]
        ...
        counts = Counter(str(row["progression_label"]) for row in selected)
        folds.append(
            {
                "fold": fold,
                "rows": len(selected),
...
def metrics_from_confusion(
    ...
    *,
    ...
    if require_all_labels and bool((support <= 0).any()):
        missing = [
            label
            for label, value in zip(labels, support, strict=True)
            if value <= 0
        ]
...
def progression_rows_from_predictions(
    ...
    *,
    ...
    for record, observation_id in zip(records, record_ids, strict=True):
        target = str(record["progression_label"])
        prediction = str(predictions[observation_id])
        if target not in label_set:
            raise ValueError(
                f"progression target is outside registered labels: {target!r}"
            )
        ...
        rows.append(
            {
                "patient_id": str(record["patient_id_hash"]),
...
def build_pair_and_entity_manifests(
    ...
    for record in records:
        ...
        key = (
            str(record["patient_id_hash"]),
            str(record["prior_study_id"]),
            str(record["current_study_id"]),
        )
    ...
    for key in sorted(groups):
        patient_id_hash, prior_study_id, current_study_id = key
        ...
        pair_manifest.append(
            {
                ...
                "findings": sorted({str(item["finding"]) for item in members}),
                "progression_counts": dict(
                    sorted(
                        Counter(
                            str(item["progression_label"]) for item in members
                        ).items()
                    )
                ),
            }
            ...
            entity_manifest.append(
                {
                    ...
                    "progression_label": str(item["progression_label"]),
                }
            )

    entity_manifest.sort(key=lambda item: item["sample_id"])
