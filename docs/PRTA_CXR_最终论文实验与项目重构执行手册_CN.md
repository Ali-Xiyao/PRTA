...
截至 2026-08-03，该 250 条门禁已按 Luna 辅助方式完成：两名临床资历均超过 5 年
的医生在可见 Luna 标签后共同形成一列共识标签，246/250（98.4%）确认 Luna，4 条
修正，250 条明确五分类全部冻结为 Gold。该 98.4% 必须称为辅助复核确认率，不能
称为独立盲审准确率或医生间一致率。
...
- 若使用 Luna 预标签辅助，必须明确报告可见预标签和潜在锚定，不得称为盲审。
...
本项目实际执行为两名临床资历均超过 5 年的医生共同填写一列 Luna 可见的共识标签。
250 条全部可判定，两个来源均为 123/125（98.4%）确认；Improved/New/Resolved/
Stable/Worse 分别为 48/50、50/50、50/50、49/50、49/50。四条修正进入 Gold，
不回填或逐条修补其余 126,727 条 Silver。该子集称为
`Luna-assisted senior-physician panel consensus Gold`，不称为独立双阅片 Gold。
...
6. 全量完成后分层抽取 250 条做人工确认/修正并冻结 Gold；
7. 若质量门失败才修订并冻结 prompt/规则后重新运行全量标签；本次确认率 98.4%，
   未修改 prompt，也未逐行修补全量 Silver；
8. 排除历史 test、医生审计集、现有 Gold，以及已冻结 Gold roster 患者的全部行；
...
- Dev：10% 患者；
- Internal test：10% 患者；
- 250 条资深医生共识 Gold：独立 quarantine；
